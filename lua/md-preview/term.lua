...
local window = require("md-preview.window")
local file = require("md-preview.utils.file")
local notify = require("md-preview.utils.notify")
...
function Term:setup()
		...
		if vim.g.mp_bufnr ~= nil then
			for _, key in pairs(keys) do
				vim.api.nvim_buf_set_keymap(
					...
					"n",
					key,
					[[<cmd>lua require('md-preview').do_action(']] .. action .. [[')<cr>]],
	...
	if term_reload_opts["enable"] then
		...
		api.nvim_exec([[
      aug MP
        au ]] .. reload_events .. [[ ]] .. aupats .. [[ execute "lua require('md-preview').refresh()"
      aug END
    ]], false)
	else
		api.nvim_exec(
...
---Open a terminal window and display viewer generated contents.
---
function Term:open()
...
---Display viewer generated contents.
---
